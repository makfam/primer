...
How do we decide which variables to include in a model? There is no one right answer to this question. To focus on one example, let's use the `trains` dataset to evaluate how helpful certain variables are to creating an effective model. 
...
What do we do when the value of `loo_compare()` is small? As a general practice, differences smaller than four are hard to distiguish from noise. In other words: when `elpd_diff` is less than 4, there is no advantage to one model over the other. 
